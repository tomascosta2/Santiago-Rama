...
export default function CalendlyEmbed({ defaultQualified = false }: { defaultQualified?: boolean }) {
  ...
  useEffect(() => {
    ...
    // Anuncio: leer param "ad" de la URL, fallback a localStorage
  ...
  useEffect(() => {
    const handleCalendlyEvent = (e: MessageEvent) => {
      ...
      // Lead: cuando la persona elige día y hora (muestra intención).
      // El pixel firma con un eventID que reusamos en CAPI al agendar (dedup).
      if (e.data?.event === "calendly.date_and_time_selected") {
        if (!leadEventIdRef.current) {
          leadEventIdRef.current = `lead-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
        }
        if (typeof window !== "undefined" && typeof (window as any).fbq === "function") {
          (window as any).fbq("track", "Lead", {}, { eventID: leadEventIdRef.current });
        }
        return;
      }
      ...
      if (e.data?.event === "calendly.event_scheduled") {
        ...
        // Sin form previo: quien completa la reserva ya pasó la calificación
        // de Calendly (los descartados se redirigen antes de agendar).
        const isQualified = localStorage.getItem("isQualified") || (defaultQualified ? "true" : null);
        ...
        const sendAll = (
          ...
          // Sin formulario previo, el nombre/email/teléfono pueden venir de Calendly (invitee).
          const currentName = nameRef.current || inviteeName || "";
          const currentEmail = emailRef.current || inviteeEmail || "";
          const currentPhone = phoneRef.current || inviteePhone || "";
        ...
        if (inviteeUri) {
          fetch("/api/calendly/invitee", {
            ...
            .then((r) => r.json())
            .then((data) => {
              const resource = data?.invitee?.resource;
              const questions = resource?.questions_and_answers ?? [];
              const startTime = data?.event?.resource?.start_time ?? undefined;
              const inviteeEmail = resource?.email ?? undefined;
              const inviteePhone = resource?.text_reminder_number ?? undefined;
              const inviteeName = resource?.name ?? undefined;
              sendAll(questions, startTime, inviteeEmail, inviteePhone, inviteeName);
            })
            .catch((err) => {
